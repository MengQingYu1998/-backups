...
export function start02() {
  ...
  $('#btnSend').click(function() {
    $.ajax({
      ...
      success: function(a) {
        ...
        if (parseInt(a) > 0) {
          $('#spanMessage').html(
            '<div class="alert-success"><strong>Done! </strong> Your message has been sent.</div>'
          )
        } else {
          alert(a)
          $('#spanMessage').html(
            '<div class="alert-error"><strong>Error! </strong> Somthing is Wrong!</div>'
          )
        }
      }
    })
  ...
  $('#btnSubmit').click(function() {
    $.ajax({
      type: 'POST',
      data: $('#frmnotify').serialize(),
      success: function(a) {
        $('#submitMessage').html('Please Wait...')
        if (parseInt(a) > 0) {
          $('#submitMessage').html(
            '<div class="alert-success"><strong>Done! </strong> Successfully submitted</div>'
          )
        } else {
          alert(a)
          $('#submitMessage').html(
  ...
  $('#wdSubmit').click(function() {
    $.ajax({
      type: 'POST',
      data: $('#worldtry').serialize(),
      success: function(a) {
        $('#worldtry #submitMessage').html('Please Wait...')
        if (parseInt(a) > 0) {
          $('#worldtry #submitMessage').html(
            '<div class="alert-success"><strong>Done! </strong> Successfully submitted</div>'
          )
        } else {
          alert(a)
...
export function start04() {
  ...
  if (
    ...
    $('#lakes')
      .find('path')
      .attr({ fill: map_config['default']['lakesColor'] })
      .css({ stroke: map_config['default']['borderColor'] })
  }
...
export function start05() {
  ...
  if (
    ...
    $('#pin-shadow')
      .find('path')
      .attr({ fill: pin_config['default']['pinShadow'] })
      .css({ 'fill-opacity': pinShadowOpacity })
  }
  ...
  if (points_len > 0) {
    ...
    for (var i = 0; i < points_len; i++) {
      if (pin_config['points'][i]['shape'] == 'circle') {
        ...
        svg_circle.setAttributeNS(
          null,
          'cx',
          pin_config['points'][i]['pos_X'] + 1
        )
        svg_circle.setAttributeNS(
          null,
          'cy',
          pin_config['points'][i]['pos_Y'] + 1
        )
        svg_circle.setAttributeNS(
          null,
          'r',
          pin_config['points'][i]['diameter'] / 2
        )
        svg_circle.setAttributeNS(
          null,
          'fill',
          pin_config['default']['pinShadow']
        )
        svg_circle.setAttributeNS(
          null,
          'style',
        ...
        svg_circle.setAttributeNS(
          null,
          'fill',
          pin_config['points'][i]['upColor']
        )
        svg_circle.setAttributeNS(
          null,
          'stroke',
          pin_config['points'][i]['outline']
        )
        svg_circle.setAttributeNS(
          null,
          'stroke-width',
          pin_config['points'][i]['thickness']
        )
        ...
        svg_rect.setAttributeNS(
          null,
          'x',
          pin_config['points'][i]['pos_X'] -
            pin_config['points'][i]['width'] / 2 +
            1
        )
        svg_rect.setAttributeNS(
          null,
          'y',
          pin_config['points'][i]['pos_Y'] -
            pin_config['points'][i]['height'] / 2 +
            1
        )
