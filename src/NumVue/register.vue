<template>
	<div class="box">
		...
		<div class="right">
			...
			<div class="inpDiv">
				<div>
					...
					<p class="telInp" :class="{'focInp':focinp,'wroInp':wroinp}">
						<input type="text" placeholder="请输入手机号" @blur="blurState(telval)"  @focus="focusState()" v-model="telval" @keyup="nullInput()"/>
					</p>
				...
				<div>
					...
					<p class="telInp veriInp" :class="{'focInp':focveri,'wroInp':wroveri}">
						<input type="text" placeholder="请输入短信验证码" @blur="blurVeri(verival)"  @focus="focusVeri()" v-model="verival" @keyup="nullInput()" maxlength="6"/>
				...
				<div>
					...
					<p class="codeInp" :class="{'focInp':focsCode,'wroInp':wroscode}">
						<input type="password" placeholder="请输入确认密码" @blur="blurCodes(codesval)"  @focus="focusCodes()"  v-model="codesval" @keyup="nullInput()" />
					</p>
			...
			<div class="goRegiste">
				<p>已有账号，<span><router-link :to="{ path: '/login'}">登录</router-link></span></p>
			</div>
...
<script>
	export default{
		...
		methods:{
			...
			nullInput(){
				if(this.telval.length>=1&&this.verival>=1&&this.codeval.length>=1&&this.codesval>=1){
					this.ishas=true;
