<template>
	<div class="box" id="rank">
		...
		<div class="title">排名上升/下降监控</span>
		</div>
		<div class="content">
			<ul>
			...
			<div class="Leibox">
				...
				<div class="ying" v-if="showApplication" >
					...
					<div class="lie">
						<span v-for="Application in Applications.Data"  @click="cliLie(Application)" :key="Application.id">{{Application.name}}</span>
					</div>
				</div>
				<!-- 游戏下拉框 -->
				<div class="game" v-show="showGame">
					<img src="../assets/NumImg/jiao.png" class="jiao" />
					<div class="lie">
						<span v-for="(game,index) in games.Data" v-html="game.name" @click="cliGame(game)" :key="game.id"></span>
					</div>
				</div>
				<!-- 日期下拉框 -->
